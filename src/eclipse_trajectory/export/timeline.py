...
def write_timeline(session_dir: Path, actions: list[ActionRecord]) -> Path:
    ...
    if session_path.exists():
        summary = json.loads(session_path.read_text(encoding="utf-8")).get("executive_summary")
    summary_html = (
        f'<section class="session-summary"><h2>Executive summary</h2><p>{html.escape(summary)}</p></section>'
...
.tag{{color:var(--accent)}} .detail,.session-summary{{color:var(--muted)}} .empty{{color:var(--muted)}} @media(max-width:700px){{.frames{{grid-template-columns:1fr}}}}
...
<p>Automated labels remain unreviewed until accepted by a qualified reviewer; unsupported clinical intent is never inferred.</p>
</header><main>{summary_html}{cards}</main></body></html>"""
...
def _action_card(action: ActionRecord) -> str:
    ...
    semantic = action.semantic_action.description or "Semantic action not identified."
    summary = action.high_level_summary or "No high-level summary."
    confidence = action.primitive_action.confidence
    confidence_text = (
        f"confidence: {confidence:.2f}" if confidence is not None else "confidence: unknown"
    )
    detail = (
        f"{html.escape(semantic)}<br>{html.escape(summary)}<br>"
        f"Episode: {html.escape(action.episode_label or 'unknown_episode')} · {confidence_text}"
    )
    return (
